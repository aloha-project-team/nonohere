...
basic_df=pd.read_csv('./last_place_info.csv',encoding="utf-8")
etc_df=pd.read_csv('./last_etc.csv',encoding="utf-8")
...
dff=pd.read_csv('./Yes.csv',encoding='utf-8')
...
# for i in range(306,389):
#     dff.iloc[i]
# pr_df=pd.read_csv('./visitor.csv',encoding="euc-kr")

# # fi_df=pd.read_csv('./No.csv',encoding="utf-8")

# # # fi_df['mean_visit'] = 0
# # # a_list=[]
# # # for df in pr_df['place']:
# # #     for df1 in fi_df['Name']:
# # #         if(df==df1):
# # #             a_list.append(pr_df[pr_df['place']==df1]['mean_visit'])

# # # # np.array(a_list)

# # # df_a['mean_visit']= np.array(a_list)
# # # # csv로 저장
# # # df_a.to_csv("./No.csv",index=False, mode='w')

# # # # fi_a['Category']= a_list